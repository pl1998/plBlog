   ...
   <LeftSidebar></LeftSidebar>
      <div class="col-sm-6 m-0" >
        <div class="card card-content" :key="list.id" v-for="list in lists">
           <h5><a class="title float-left text-left" :href="`/article/`+list.id">{{list.title}}</a></h5>
           <div class="label">
                 <a @click="setKeyword(list.label)" class="btn btn-danger float-left">{{list.label}}</a>
            </div>
           <div class="row bg">
             <div class="col-8 text-left description">
               {{list.description}}
           </div>
             <div class="col-4">
           ...
           <span class="float-left">
            <i class="fa fa-calendar mr-2"></i>{{list.created_at}}&nbsp;&nbsp;
          </span>
           <span class="float-left">
            <i class="fa fa-eye mr-2"></i>{{list.review_count}}&nbsp;&nbsp;
          </span>
           <span class="float-left">
            <i class="fa fa-comments mr-2"></i>{{list.browse_count}}&nbsp;&nbsp;
          </span>
           </div>
        </div>
         <Pagination :currentPage="currentPage" :total="total" @handleCurrentChange="handleCurrentChange" @handleSizeChange="handleSizeChange"/>
      </div>
...
export default {
  ...
  data(){
    return {
      pageSize: 10,
      currentPage:parseInt(this.$route.query.page) ? parseInt(this.$route.query.page) :1,
      total: 0,
      lists:[],
      data: {
        pageSize: 10,
  ...
  methods:{
    ...
    setKeyword(val){
      this.$router.push({path: '/#', query: {page:this.currentPage,keywords: val}})
